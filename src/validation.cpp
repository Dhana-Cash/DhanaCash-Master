...
#if defined(NDEBUG)
# error "DhanaCash cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in DhanaCash
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
        ...
        Params().NetworkIDString() == CBaseChainParams::TESTNET)
    {
        if (fSuperblockPartOnly) {
            if (nPrevHeight < 2000000)      return 220000 * COIN;
            else                            return 220000 * COIN;
        }
        ...
        if (nPrevHeight + 1 < Params().GetConsensus().DIP0003EnforcementHeight)
            ...
            return mnPayment + (4000 * COIN);
    }

    // proof of work
    CAmount nSubsidyBase = 4000;
    CAmount nSubsidy = nSubsidyBase * COIN;

    if (nPrevHeight <= 1) {
        nSubsidy = 14850000000 * COIN;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight > Params().GetConsensus().nLastPoWBlock ||
        Params().NetworkIDString() == CBaseChainParams::TESTNET)
    {
        if (nHeight < 2000000)      return 1000 * COIN;
        else                        return 1000 * COIN;
    }

    // proof of work
    CAmount ret = blockValue * 0.2; // start at 20%
    return ret;
}
...
void ThreadScriptCheck() {
    RenameThread("dhanacash-scriptch");
...
static int64_t nTimeDhanaCashSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeDhanaCashSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - DhanaCash specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeDhanaCashSpecific * 0.000001);
