...
void TransactionRecord::updateStatus(const CWalletTx &wtx, int chainLockHeight)
{
    ...
    {
        // The IsLockedByInstaDCAS call is quite expensive, so we only do it when a state change is actually possible.
        if (status.lockedByChainLocks) {
